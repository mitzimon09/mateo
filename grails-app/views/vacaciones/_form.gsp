...
<div class="fieldcontain ${hasErrors(bean: vacaciones, field: 'solicitudSalida', 'error')} ">
	<label for="solicitudSalida">
		<g:message code="vacaciones.solicitudSalida.label" default="Solicitud Salida" />
		
	</label>
	<g:select id="solicitudSalida" name="solicitudSalida.id" from="${mx.edu.um.rh.SolicitudSalida.list()}" optionKey="id" value="${vacaciones?.solicitudSalida?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
