			...
			<table>
				...
				<g:each in="${eventoInstanceList}" status="i" var="eventoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: eventoInstance, field: "prorroga")}</td>
					
						<td>${fieldValue(bean: eventoInstance, field: "status")}</td>
					
						<td><g:formatDate date="${eventoInstance.hora_final}" /></td>
					
						<td><g:formatDate date="${eventoInstance.hora_inicio}" /></td>
