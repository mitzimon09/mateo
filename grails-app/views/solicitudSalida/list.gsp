	...
	<body>
		...
		<div id="list-solicitudSalida" class="content scaffold-list" role="main">
			...
			<table>
				...
				<tbody>
				<g:each in="${solicitudSalidaList}" status="i" var="solicitudSalida">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					
						<td><g:link action="ver" id="${solicitudSalida.id}">${fieldValue(bean: solicitudSalida, field: "status")}</g:link></td>
					
						<td>${fieldValue(bean: solicitudSalida, field: "observaciones")}</td>
					
						<td>${fieldValue(bean: solicitudSalida, field: "empresa")}</td>
					
						<td>${fieldValue(bean: solicitudSalida, field: "vacaciones")}</td>
					
						<td>${fieldValue(bean: solicitudSalida, field: "empleado")}</td>
					
						<td><g:formatDate date="${solicitudSalida.dateCreated}" /></td>
			...
			<div class="pagination">
				<g:paginate total="${solicitudSalidaTotal}" />
			</div>
		</div>
	</body>
</html>
